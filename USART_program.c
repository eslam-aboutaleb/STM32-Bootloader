...
static StatusTypeDef UART_WaitOnFlagUntilTimeout(USART_Handle *huart, uint32 Flag, FlagState Status, uint32 Tickstart, uint32 Timeout);
...
uint8 UART_GetChar(USART_Handle *USART_Handle)
{
	...
	return ((uint8)(USART_Handle->Instance->DR & 0x00FF));
}

StatusTypeDef UART_Recieve(USART_Handle *USART_Handle,uint8 *pData, uint16 Size, uint32 TimeOut)
{
	uint32 TickStart=0U;
	
	_LOCK(USART_Handle);
	
	USART_Handle->RxXferCount = Size;
	USART_Handle->RxXferSize = Size;
	
	TickStart=Sys_GetTick();
	
	_UNLOCK(USART_Handle);
	
	while(USART_Handle->RxXferCount > 0U)
	{
		USART_Handle->RxXferCount--;
		if(UART_WaitOnFlagUntilTimeout(USART_Handle,(uint32)USART_SR_RXNE_Msk, FlagRESET, TickStart, TimeOut) != OK)
    { 
          return TIMEOUT;
    }
		else
		{
		*pData++ = (uint8)(USART_Handle->Instance->DR & (uint8)0x00FF);
...
static StatusTypeDef UART_WaitOnFlagUntilTimeout(USART_Handle *huart, uint32 Flag, FlagState Status, uint32 Tickstart, uint32 Timeout)
{
	while((UART_GET_FLAG(huart, Flag) ? FlagSET : FlagRESET) == Status) 
  {
    /* Check for the Timeout */
    if(Timeout != MAX_DELAY)
    {
      if((Timeout == 0U)||((Sys_GetTick() - Tickstart ) > Timeout))
      {
        /* Disable TXE, RXNE, PE and ERR (Frame error, noise error, overrun error) interrupts for the interrupt process */
        CLEAR_BIT(huart->Instance->CR1, (5 | 8 | 7));
        CLEAR_BIT(huart->Instance->CR3, 0);
...
void UART_WriteString(USART_Handle *USART_Handle,uint8 * string)
{
while(*string!='\0')
	{
	UART_SendChar(USART_Handle,*string);
	string++;
	}
	UART_SendChar(USART_Handle,'\0');
}


void UART_ReadString(USART_Handle *USART_Handle,uint8 * string)
{
	/*temporary variable as a buffer*/
	uint8 data;
	while(1) 
	{
	    data=UART_GetChar(USART_Handle);
